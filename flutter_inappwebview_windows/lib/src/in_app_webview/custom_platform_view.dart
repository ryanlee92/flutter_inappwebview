...
enum InAppWebViewKeyEventKind { keyDown, keyUp, character }
...
class CustomPlatformViewController extends ValueNotifier<CustomFlutterViewControllerValue> {
  ...
  Future<void> _setPointerUpdate(InAppWebViewPointerEventKind kind, int pointer, Offset position, double size, double pressure) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
  ...
  Future<void> _setPointerButtonState(InAppWebViewPointerEventKind kind, PointerButton button) async {
    ...
    assert(value.isInitialized);
    return _methodChannel.invokeMethod('setPointerButton', <String, dynamic>{'kind': kind.index, 'button': button.index});
  }
  ...
  Future<void> _sendKeyEvent({
    required InAppWebViewKeyEventKind kind,
    required int virtualKey,
    required int scanCode,
    required bool isExtendedKey,
    required bool isMenuKeyDown,
    required bool wasKeyDown,
    required bool isKeyReleased,
  }) async {
...
class CustomPlatformView extends StatefulWidget {
  ...
  const CustomPlatformView({this.creationParams, this.onPlatformViewCreated, this.scaleFactor, this.filterQuality = FilterQuality.none});
  ...
  Widget build(BuildContext context) {
    return RawKeyboardListener(
      ...
      onKey: (RawKeyEvent event) {
        ...
        final bool isUp = event is RawKeyUpEvent;

        final bool isMenu =
            (data.modifiers & RawKeyEventDataWindows.modifierAlt) != 0 ||
            (data.modifiers & RawKeyEventDataWindows.modifierLeftAlt) != 0 ||
            (data.modifiers & RawKeyEventDataWindows.modifierRightAlt) != 0;

        // Send KEY_DOWN/KEY_UP
        unawaited(
          _controller._sendKeyEvent(
            kind: isDown ? InAppWebViewKeyEventKind.keyDown : InAppWebViewKeyEventKind.keyUp,
            virtualKey: data.keyCode,
            scanCode: data.scanCode,
            isExtendedKey: false,
            isMenuKeyDown: isMenu,
            wasKeyDown: !isDown && isUp,
            isKeyReleased: isUp,
          ),
        );

        // Optionally send CHARACTER on key down with a character
        if (isDown && event.character != null && event.character!.isNotEmpty) {
          final ch = event.character!.codeUnitAt(0);
          unawaited(
            _controller._sendKeyEvent(
              kind: InAppWebViewKeyEventKind.character,
              virtualKey: ch,
              scanCode: data.scanCode,
              isExtendedKey: false,
              isMenuKeyDown: isMenu,
            ...
            ? Listener(
                ...
                onPointerDown: (ev) {
                  ...
                  if (ev.kind == PointerDeviceKind.touch) {
                    _controller._setPointerUpdate(InAppWebViewPointerEventKind.down, ev.pointer, ev.position, ev.size, ev.pressure);
                    return;
                  }
                ...
                onPointerUp: (ev) {
                  _pointerKind = ev.kind;
                  if (ev.kind == PointerDeviceKind.touch) {
                    _controller._setPointerUpdate(InAppWebViewPointerEventKind.up, ev.pointer, ev.position, ev.size, ev.pressure);
                    return;
                  }
                ...
                onPointerMove: (ev) {
                  _pointerKind = ev.kind;
                  if (ev.kind == PointerDeviceKind.touch) {
                    _controller._setPointerUpdate(InAppWebViewPointerEventKind.update, ev.pointer, ev.position, ev.size, ev.pressure);
                  } else {
                    _controller._setCursorPos(ev.position);
                  }
                },
